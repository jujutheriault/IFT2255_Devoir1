...
La conception du système utilise une architecture structurée autour d'une séparation claire entre modèle, contrôleurs et vues (MVC), ce qui favorise une compréhension simple du fonctionnement global et une évolue simple du logiciel. Elle adopte une architecture modulaire en couches exposant une API REST. Le choix du modulaire s'est fait par le fait que le projet est crée par une petite équipe et le modèle présente une simplicité de développement et de déploiement. 
